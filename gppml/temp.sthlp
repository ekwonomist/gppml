...
{pstd}
{cmd:cvmrtest} is a post-estimation command designed to be run after {cmd:ppmlhdfe}. It estimates the optimal value of λ using an Iterated Generalized Method of Moments (GMM) procedure. The estimated value is stored in the return list as {cmd:e(lambda)}, which can then be used with the {cmd:gppmlhdfe} command for the final estimation. λ is a variance parameter in the conditional variance function, such that Var(Y|x) = h*μ^(λ), where μ is the conditional mean of the dependent variable E(Y|x).
